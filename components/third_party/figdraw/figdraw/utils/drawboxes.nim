...
proc drawOuterBox*[R](ctx: R, rect: Rect, padding: float32, color: Color) =
  ...
  let rectRight =
    rect(xy + vec2(obox.w - padding, padding), vec2(padding, obox.h - 2 * padding))
  ...
  block drawCorners:
    ...
    let
      ...
      coffset = [
        ...
        dcBottomRight: vec2(
          cornerCbs[dcBottomRight].sideDelta.float32,
          cornerCbs[dcBottomRight].sideDelta.float32,
        ),
      ]
      ...
      angles =
        [dcTopLeft: 0.0, dcTopRight: -Pi / 2, dcBottomLeft: Pi / 2, dcBottomRight: Pi]

...
proc loadInt*(
    gen: var IRGenerator, position: uint, value: MAtom
): uint {.inline, discardable.} =
  ## Load an stackInteger into the memory space.
...
proc loadList*(gen: var IRGenerator, position: uint): uint {.inline, discardable.} =
...
proc appendList*(
    gen: var IRGenerator, dest, source: uint
): uint {.inline, discardable.} =
...
proc jump*(gen: var IRGenerator, position: uint): uint {.inline, discardable.} =
...
proc loadStr*(
    gen: var IRGenerator, position: uint, value: string | MAtom
): uint {.inline, discardable.} =
...
proc loadNull*(gen: var IRGenerator, position: uint): uint {.inline, discardable.} =
  ## Load a NULL atom into memory.
  gen.addOp(IROperation(opCode: LoadNull, arguments: @[stackInteger position]))

proc readRegister*(
    gen: var IRGenerator, position: int | uint, register: Register
): uint {.inline, discardable.} =
...
proc readRegister*(
    gen: var IRGenerator, position: uint, index: uint, register: Register
): uint {.inline, discardable.} =
...
proc loadUint*[P: SomeInteger](
    gen: var IRGenerator, position: uint, value: P
): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(
      opCode: LoadUint,
...
proc loadUint*(
    gen: var IRGenerator, position: uint | int, value: MAtom
): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(opCode: LoadUint, arguments: @[stackInteger position.uint, value])
  )

proc returnFn*(gen: var IRGenerator, position: int = -1): uint {.inline, discardable.} =
...
proc loadBool*(
    gen: var IRGenerator, position: uint, value: bool | MAtom
): uint {.inline, discardable.} =
...
proc call*(
    gen: var IRGenerator, function: string, arguments: seq[MAtom] = @[]
): uint {.inline, discardable.} =
...
proc loadObject*(gen: var IRGenerator, position: uint): uint {.inline, discardable.} =
...
# "slow"
proc writeField*(
    gen: var IRGenerator, position: uint, name: string, value: uint
): uint {.inline, discardable.} =
...
# "fast"
proc writeField*(
    gen: var IRGenerator, position: uint, index: int, value: uint
): uint {.inline, discardable.} =
  ## Modify a field of an object.
...
proc incrementInt*(gen: var IRGenerator, position: uint): uint {.inline, discardable.} =
...
proc decrementInt*(gen: var IRGenerator, position: uint): uint {.inline, discardable.} =
...
proc zeroRetval*(gen: var IRGenerator): uint {.inline, discardable.} =
  gen.addOp(IROperation(opCode: ZeroRetval))

proc placeholder*(gen: var IRGenerator, opCode: Ops): uint {.inline, discardable.} =
  gen.addOp(IROperation(opCode: opCode))

proc overrideArgs*(
    gen: var IRGenerator, instruction: uint, arguments: seq[MAtom]
) {.inline.} =
  for i, _ in gen.modules:
    var module = gen.modules[i]
    if module.name == gen.currModule:
...
proc add*(gen: var IRGenerator, dest, source: uint): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(opCode: Add, arguments: @[stackInteger dest, stackInteger source])
  )

proc mult*(gen: var IRGenerator, dest, source: uint): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(opCode: Mult, arguments: @[stackInteger dest, stackInteger source])
  )

proc divide*(gen: var IRGenerator, dest, source: uint): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(opCode: Div, arguments: @[stackInteger dest, stackInteger source])
  )

proc sub*(gen: var IRGenerator, dest, source: uint): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(opCode: Sub, arguments: @[stackInteger dest, stackInteger source])
  )

proc passArgument*(gen: var IRGenerator, position: uint): uint {.inline, discardable.} =
...
proc loadFloat*(
    gen: var IRGenerator, position: uint, value: float | MAtom
): uint {.inline, discardable.} =
...
proc moveAtom*(gen: var IRGenerator, source, dest: uint): uint {.inline, discardable.} =
...
proc copyAtom*(gen: var IRGenerator, source, dest: uint): uint {.inline, discardable.} =
  ## Copy an atom from one index to another.
  gen.addOp(
    IROperation(opCode: CopyAtom, arguments: @[stackInteger source, stackInteger dest])
  )

proc resetArgs*(gen: var IRGenerator): uint {.inline, discardable.} =
  ## Reset the call arguments register.
  gen.addOp(IROperation(opCode: ResetArgs))

proc lesserThan*(gen: var IRGenerator, a, b: uint): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(opcode: LesserThanInt, arguments: @[stackInteger a, stackInteger b])
  )

proc greaterThan*(gen: var IRGenerator, a, b: uint): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(opcode: GreaterThanInt, arguments: @[stackInteger a, stackInteger b])
  )

proc greaterThanEqual*(gen: var IRGenerator, a, b: uint): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(
...
proc lesserThanEqual*(gen: var IRGenerator, a, b: uint): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(
...
proc loadBytecodeCallable*(
    gen: var IRGenerator, index: uint, clause: string
): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(
...
proc invoke*(gen: var IRGenerator, index: uint): uint {.inline, discardable.} =
  gen.addOp(IROperation(opcode: Invoke, arguments: @[stackInteger(index)]))

proc throwReferenceError*(
    gen: var IRGenerator, binding: string
): uint {.inline, discardable.} =
  gen.addOp(IROperation(opcode: ThrowReferenceError, arguments: @[stackStr(binding)]))

proc resolveField*(
    gen: var IRGenerator, address: uint, accessResult: uint, access: seq[string]
): uint {.discardable, inline.} =
  gen.addOp(
    IROperation(
      opcode: ResolveField,
      arguments:
        @[stackInteger(address), stackInteger(accessResult)] & access.mapIt(
          stackStr(it)
        ),
    )
  )

proc passMultipleArguments*(
    gen: var IRGenerator, positions: seq[uint]
): uint {.inline, discardable.} =
  gen.addOp(
    IROperation(
      opCode: PassMultipleArguments,
      arguments: positions.mapIt(stackInteger(cast[int64](it))),
    )
  )

proc setThisBinding*(
    gen: var IRGenerator, binding: uint
): uint {.inline, discardable.} =
  gen.addOp(IROperation(opCode: SetThisBinding, arguments: @[stackInteger(binding)]))

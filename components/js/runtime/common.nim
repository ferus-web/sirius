...
proc typeRegistrationFinalizer*(runtime: Runtime) {.gcsafe.} =
  ...
  for typ in runtime.types:
    ...
    if typ.constructor != nil:
      let fn = nativeCallable(runtime.realm.heap, typ.constructor)
...
proc registerEcmaTypes*(runtime: Runtime) =
  ...
  std_function.generateBindings(runtime)
  std_array.generateBindings(runtime)

  promise.generateBindings(runtime)
...
proc run*(runtime: Runtime) {.gcsafe.} =
  ...
  runtime.vm.heapManager = runtime.realm.heap

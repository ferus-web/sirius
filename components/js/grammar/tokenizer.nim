...
func consume*(tokenizer: var Tokenizer): char =
  inc tokenizer.pos
...
func next*(tokenizer: var Tokenizer): Token
...
func consumeInvalid*(tokenizer: var Tokenizer): Token =
  tokenizer.advance()
...
func charToDecimalDigit*(c: char): Option[uint32] {.inline.} =
...
func consumeBackslash*(tokenizer: var Tokenizer): Result[char, MalformedStringReason] =
  tokenizer.advance()
...
func consumeIdentifier*(tokenizer: var Tokenizer): Token =
  var
    ident = newString(0)
...
func consumeWhitespace*(tokenizer: var Tokenizer): Token {.inline.} =
  var ws = newString(0)
...
func consumeEquality*(tokenizer: var Tokenizer): Token =
  tokenizer.advance()
...
func consumeString*(tokenizer: var Tokenizer): Token =
...
func consumeComment*(tokenizer: var Tokenizer, multiline: bool = false): Token =
  tokenizer.advance()
...
func consumeSlash*(tokenizer: var Tokenizer): Token {.inline.} =
  tokenizer.advance()
...
func consumeExclaimation*(tokenizer: var Tokenizer): Token =
  tokenizer.advance()
...
func consumeAmpersand*(tokenizer: var Tokenizer): Token {.inline.} =
  tokenizer.advance()
  let next = tokenizer.charAt()
  if !next:
...
func consumePipe*(tokenizer: var Tokenizer): Token {.inline.} =
  tokenizer.advance()
...
func consumeHash*(tokenizer: var Tokenizer): Token =
  tokenizer.advance()
...
func consumeGreaterThan*(tokenizer: var Tokenizer): Token {.inline.} =
  tokenizer.advance()
...
func consumeLessThan*(tokenizer: var Tokenizer): Token {.inline.} =
  tokenizer.advance()
...
func consumeMinus*(tokenizer: var Tokenizer): Token {.inline.} =

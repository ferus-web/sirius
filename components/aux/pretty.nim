## treeform's `pretty` library, forked to make it stop emitting annoying rune tuples. Also removes JS backend support.
##
## Copyright (C) 2023-2026 Andre von Houck
## Copyright (C) 2026 Trayambak Rai (xtrayambak@disroot.org)

import std/[macros, sets, strutils, typetraits, math, tables, json, terminal, unicode]
...
proc add*(ctx: PrettyContext, name: string, v: JsonNode) =
  ...
  of JString:
    ctx.parent.nodes.add Node(
      kind: StringNode, name: name, value: escapeString(v.getStr)
    )
  of JInt, JFloat:
    ctx.parent.nodes.add Node(kind: NumberNode, name: name, value: $v)
  of JBool:
...
macro prettyWalk*(n: varargs[untyped]): untyped =
  var command = nnkStmtList.newTree(
    nnkLetSection.newTree(
      nnkIdentDefs.newTree(
        ...
        newEmptyNode(),
        nnkCall.newTree(newIdentNode("newPrettyContext")),
      )
    )
  )
  ...
  for i in 0 .. n.len - 1:
    if n[i].kind == nnkStrLit:
      command.add nnkCall.newTree(
        nnkDotExpr.newTree(newIdentNode("context"), newIdentNode("add")), n[i]
      )
    elif n[i].kind == nnkIdent:
      command.add nnkCall.newTree(
        nnkDotExpr.newTree(newIdentNode("context"), newIdentNode("add")),
        newLit(n[i].repr),
        n[i],
      )
    else:
  ...
  return nnkBlockStmt.newTree(newEmptyNode(), command)
...
template prettyFieldPairs*(x: untyped): untyped =
  when compiles(x[]): x[].fieldPairs else: x.fieldPairs
...
proc printBarChart*[N: SomeNumber](data: seq[(string, N)]) =
  ...
  ##
  const fillChar = "#"
  proc maximize(a: var SomeNumber, v: SomeNumber) =
    a = max(a, v)

  proc minimize(a: var SomeNumber, v: SomeNumber) =
    a = min(a, v)

  proc frac(a: SomeFloat): SomeFloat =
    a - floor(a)
  ...
  var chartWidth = lineWidth - maxKeyWidth - 3 - maxLabel - 2
  if minNumber != 0:
